...
First to apply a kernel to a part of and image we use the convolution funtion. This function return the result color of the convolution. 
...
> :P5 sketch=/docs/sketches/workshop1/w2/convolutionVideo.js, width=320, height=500
...
function draw() {
  ...
  lienzo.updatePixels();
  image(fingers,1,0); 
  image(lienzo,1,fingers.height+1);
}
